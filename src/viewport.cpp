...
const ViewportDrawer* GetViewportDrawer() { return &_vd; }
...
void ViewportDoDraw(const Viewport &vp, int left, int top, int right, int bottom)
{
	...
	/* The DPI left/top might be slightly different than the requested coordinates due to rounding. */
	ZoomLevel zoom = _vd.dpi.zoom;
	DrawPixelInfo dp = _vd.dpi;

	if (Renderer3D::IsEnabled()) {
		Renderer3D::Render(vp, _vd.dpi);
	}
